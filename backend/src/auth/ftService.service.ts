...
export class FtService
{
    ...
    async getProfileFromCode(code: string)
    {
        const res = await fetch('https://api.intra.42.fr/oauth/token', {
            method: 'POST',
            body: new URLSearchParams({
    ...
    private async getProjectName(projectId: string): Promise<string>
    {
        ...
        const res = await fetch(`https://api.intra.42.fr/v2/projects/${projectId}`, {
    ...
    async syncUserTeam(userId: string)
    {
        ...
        while (true)
        {
            ...
            {
                const memberFtIds = team.users.map((u: any) => String(u.id));
                if (memberFtIds.length === 1 || team["validated?"] === false) //todo change || team["validated?"] === false for !== null
                    continue ;
